...
interface RectangularShapeProps {
  color: string;
  w: string;
}

const RectangularShape: React.FC<RectangularShapeProps> = ({ color, w }) => {
...
interface CicleShapesProps {
  color: string;
}

const CicleShapes: React.FC<CicleShapesProps> = ({ color }) => {
  return (
    <div className="relative w-full h-full">
      <motion.svg
        {...AnimateCircle}
        viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="w-full h-full absolute">
        <path
      ...
      <motion.div
        ...
        className="w-[20%] h-[20%] rounded-full absolute top-[16%] left-[16%]"
